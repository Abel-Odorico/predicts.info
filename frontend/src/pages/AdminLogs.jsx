...
export default function AdminLogs() {
  ...
  const load = useCallback(() => {
    setLoading(true)
    api.get('/admin/client-diag?limit=300', token).then(setRows).catch(() => setRows([])).finally(() => setLoading(false))
  }, [token])
    ...
    <div className="adm-shell">
      <div className="adm-header">
        <div className="adm-header__left">
          ...
          <div className="adm-header__sub">predicts.info · erros JS reportados pelo navegador do usuário</div>
        </div>
      ...
      <div className="adm-pane fade-in-1">
        <p style={{ color: 'var(--text-4)', fontSize: 12.5, marginBottom: 14 }}>
          Captura global de erro JS + promise rejeitada, em qualquer página, de qualquer navegador. Guarda os últimos
          ~3000 eventos, sem tabela dedicada.
        </p>

        {rows.length === 0 && !loading && (
          <div className="bet-empty" style={{ color: 'var(--text-3)' }}>Nenhum erro registrado.</div>
        )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {rows.map((r, i) => {
            ...
            return (
              <div key={i} className="adm-card" style={{ padding: '10px 14px', cursor: 'pointer', borderColor: 'var(--lose)' }} onClick={() => setOpenRow(isOpen ? null : i)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: 'var(--font-cond)', fontSize: 13 }}>
                  ...
                  <span style={{ fontWeight: 700 }}>{r.message || '—'}</span>
